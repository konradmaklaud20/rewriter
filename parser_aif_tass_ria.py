...
with open('paral_two_aif_tass_ria.csv', 'a', encoding='utf8') as f:
...
for i in range(1, 20000):
    try:
        print("i: ", i)
        r5 = requests.post('https://rzn.aif.ru/news', data={'page': i})
        s = BeautifulSoup(r5.text, 'lxml')
        all_links = [i.find('a').get('href') for i in s.find_all('div', class_='box_info')]
        title_aif = [i.find('h3').text.strip() for i in s.find_all('div', class_='box_info')]
        date_aif = [i.find('span', class_='text_box__date').text.strip() for i in s.find_all('div', class_='box_info')]

        for num, title in enumerate(title_aif):
            search_ria = title
            try:
                r2 = requests.get('https://ria.ru/services/search/getmore/?query={}&tags_limit=20&sort=relevance'.format(search_ria))
                ...
                print(date_ria)
                if date_ria == date_aif[num][3:10] and 'Дайджест' not in title_ria:
                    ...
                    r_aif = requests.get(all_links[num])
                    s_aif = BeautifulSoup(r_aif.text, 'lxml')
                    text_aif = s_aif.find('div', class_='article_text').text.strip().replace('\n', ' ')
                    if len(text_ria) < 3000 and len(text_aif) < 3000:
                        data = {'text1': text_ria.replace('\xa0', ' '), 'text2': text_aif.replace('\xa0', ' ')}
                        write_csv(data)
            ...
            try:
                ...
                h = {"type": ["default", "article", "event"], "sections": [], "searchStr": search, "sort": "score",
                     "from": 0, "size": 20}
                ...
                print(date_tass)
                if date_tass == date_aif[num][3:10]:

                    r_aif2 = requests.get(all_links[num])
                    s_aif2 = BeautifulSoup(r_aif2.text, 'lxml')
                    text_aif2 = s_aif2.find('div', class_='article_text').text.strip().replace('\n', ' ')
                    ...
                    if len(text2) > 10:
                        ...
                        if len(text2) < 3000 and len(text_aif2) < 3000:
                            data = {'text1': text2.replace('\xa0', ' '), 'text2': text_aif2.replace('\xa0', ' ')}
                            write_csv(data)
